...
class SPSMDEffectiveCopulaBaseMass(SinglePeakSmoothedMassDistribution):
    """
    PL+P mass model with correlated spin model. This being a skewed gaussian chi_eff
    and chi_dif, with identical beta distributions for rho_1 and rho_2 and a
    copula correlating mass1 and chi_eff.
    """
    def __init__(self, mmin=2, mmax=100):
        super(SPSMDEffectiveCopulaBaseMass, self).__init__(mmin, mmax)
        self.chi_effs = xp.linspace(-1, 1, 500)
    
    def __call__(self, dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m,
                 xi_chi_eff, omega_chi_eff, chi_eff_min, chi_eff_max, chi_eff_skew, kappa_m_chi_eff, 
                 xi_chi_dif, omega_chi_dif, chi_dif_min, chi_dif_max, chi_dif_skew,
                 alpha_rho, beta_rho, amax,
                 lambda_chi_peak=0):
        """
        ...
        chi_eff_skew: float
            Skewness of chi_eff Gaussian.
        kappa_m_chi_eff: float
            Correlation between chi_eff and mass1.
        xi_chi_dif, omega_chi_dif: float
            Shape parameters of chi_dif distribution.
        chi_dif_min: float
            Minimum allowed chi_dif.
        ...
        amax: float
            Maximum allowed spin magnitude.
        """ 
        p_mass = super(SPSMDEffectiveCopulaBaseMass, self).__call__(
            dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m)
    ...
    def p_chi_eff(self, dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m,
        ...
        p_chi_eff = truncskewnorm(dataset["chi_eff"], xi=xi_chi_eff, omega=omega_chi_eff,
                                  high=chi_eff_max, low=chi_eff_min, skew=chi_eff_skew)
        p_chi_eff /= chi_eff_norm
        p_chi_eff *= self.copula_function(u, v, kappa_m_chi_eff)
        return p_chi_eff
...
class SPSMDEffectiveFrankCopulaMass(SPSMDEffectiveCopulaBaseMass):
    """
    SPSMDEffectiveCopulaBaseMass model with Frank copula density.
    """
    def copula_function(self, u, v, kappa):
        return frank_copula(u, v, kappa)
...
class SPSMDEffectiveCopulaBaseBoth(SinglePeakSmoothedMassDistribution):
    """
    PL+P mass model with correlated spin model. This being a skewed gaussian chi_eff
    and chi_dif, with identical beta distributions for rho_1 and rho_2 and a
    copula correlating mass1, mass ratio and chi_eff.
    """
    def __init__(self, mmin=2, mmax=100):
        super(SPSMDEffectiveCopulaBaseBoth, self).__init__(mmin, mmax)
        self.chi_effs = xp.linspace(-1, 1, 500)
    
    def __call__(self, dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m,
                 xi_chi_eff, omega_chi_eff, chi_eff_min, chi_eff_max, chi_eff_skew, kappa_m_chi_eff, kappa_q_chi_eff,
                 xi_chi_dif, omega_chi_dif, chi_dif_min, chi_dif_max, chi_dif_skew,
                 alpha_rho, beta_rho, amax,
                 lambda_chi_peak=0):
        """
        ...
        amax: float
            Maximum allowed spin magnitude.
        """ 
        p_mass = super(SPSMDEffectiveCopulaBaseBoth, self).__call__(
            dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m)
    ...
    def p_spin(self, dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m,
        ...
        p_spin = self.p_chi_eff(dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m,
                                xi_chi_eff, omega_chi_eff, chi_eff_max, chi_eff_min, chi_eff_skew,
                                kappa_m_chi_eff, kappa_q_chi_eff)
        p_spin *= self.p_chi_dif(dataset, xi_chi_dif, omega_chi_dif, chi_dif_max, chi_dif_min, chi_dif_skew)
        p_spin *= self.p_rho(dataset, alpha_rho, beta_rho, amax)
        return p_spin
    ...
    def p_chi_eff(self, dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m,
                  xi_chi_eff, omega_chi_eff, chi_eff_max, chi_eff_min, chi_eff_skew,
                  kappa_m_chi_eff, kappa_q_chi_eff):
        u, v, k, chi_eff_norm = self.copula_coords(dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m,
                                                   xi_chi_eff, omega_chi_eff, chi_eff_max, chi_eff_min, chi_eff_skew)
        p_chi_eff = truncskewnorm(dataset["chi_eff"], xi=xi_chi_eff, omega=omega_chi_eff,
                                  high=chi_eff_max, low=chi_eff_min, skew=chi_eff_skew)
        p_chi_eff /= chi_eff_norm
        p_chi_eff *= self.copula_function(u, v, kappa_q_chi_eff)
        p_chi_eff *= self.copula_function(k, v, kappa_m_chi_eff)
        return p_chi_eff
    ...
    def copula_coords(self, dataset, alpha, beta, mmin, mmax, lam, mpp, sigpp, delta_m,
        ...
        # u(q) grid
        k = cumtrapz(p_m, self.m1s, initial=0)
        k /= xp.max(k)
        k = xp.nan_to_num(k)

        # Interpolate for u(q)
        res_k = xp.interp(dataset["mass_1"], self.m1s, k)
        ...
        return res_u, res_v, res_k, chi_eff_norm


class SPSMDEffectiveFrankCopulaBoth(SPSMDEffectiveCopulaBaseBoth):
    """
    SPSMDEffectiveCopulaBaseBoth model with Frank copula density.
    """
    def copula_function(self, u, v, kappa):
        return frank_copula(u, v, kappa)